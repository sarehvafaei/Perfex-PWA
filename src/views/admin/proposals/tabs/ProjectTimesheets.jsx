...
//import { getProjectTimesheets } from "api/projects";

export default function ProjectTimesheets({ projectId }) {
  const [milestones, setTimesheets] = useState([]);
  const [loading, setLoading] = useState(true);

  const load = async () => {
    setLoading(true);
    try {
      const data = await getProjectTimesheets(projectId);
      setTimesheets(data.milestones || []);
    } catch (err) {
      ...
      <div className="">
        <table className="w-full text-sm">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-4 py-2 text-left border border-gray-200">Timesheet</th>
